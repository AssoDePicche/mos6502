    ...
    for (int i = 0; i < forward_ref_count; ++i) {
        ...
        if (get_label_address(forward_refs[i].label_name, &label_addr)) {
            if (forward_refs[i].type == REF_TYPE_ABS_ADDR) {
                cpu.BUS[forward_refs[i].address] = (uint8_t)(label_addr & 0xFF);

                cpu.BUS[forward_refs[i].address + 1] = (uint8_t)((label_addr >> 8) & 0xFF);
                ...
                cpu.BUS[forward_refs[i].address] = (uint8_t)(offset & 0xFF);
            }
        } else {
...
program:
    lines
    {
        ...
        while (cpu.PC < MOS6502_BUS_SIZE && cpu.PC != 0xFFFF) {
            mos6502_simulate_instruction();
        }
...
instruction:
    LDX_OP HASH immediate_operand {
        cpu.BUS[current_address++] = 0xA2;
        cpu.BUS[current_address++] = (uint8_t)$3;
    }
    | LDA_OP label_name COMMA REG_X {
        cpu.BUS[current_address++] = 0xBD;
    ...
    | STA_OP address_operand {
        cpu.BUS[current_address++] = 0x8D;

        cpu.BUS[current_address++] = (uint8_t)($2 & 0xFF);

        cpu.BUS[current_address++] = (uint8_t)(($2 >> 8) & 0xFF);
    }
    | INX_OP {
        cpu.BUS[current_address++] = 0xE8;
    }
    | JMP_OP address_operand {
        cpu.BUS[current_address++] = 0x4C;
...
byte_item:
    HEX_VALUE {
        cpu.BUS[current_address++] = (uint8_t)$1;
    }
    | DEC_VALUE {
        cpu.BUS[current_address++] = (uint8_t)$1;
    }
    | STRING_LITERAL {
        for (int i = 0; i < strlen($1); ++i) {
            cpu.BUS[current_address++] = (uint8_t)$1[i];
        }
